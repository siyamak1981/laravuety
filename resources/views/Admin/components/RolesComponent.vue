      ...
      <v-toolbar flat color="dark">
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="deep-purple accent-4" dark class="mr-4" v-on="on">New Role</v-btn>
...
<script>
  ...
  methods: {
    ...
    deleteAll(){
        ...
        if(decided){
        this.loading=true
        axios.post('auth/roles/delete', { 'roles':this.selected })
        .then(response =>{
    ...
    paginate(event){
      ...
      axios.get(`/auth/roles?page=${event.page}`, {params:{'per_page':event.itemsPerPage}})
        .then(response =>{
    ...
    initialize () {
      ...
      deleteItem (item) {
        ...
        if(decided){
        this.loading=true
        axios.delete(`/auth/roles/${item.id}`)
        .then(response =>{
      ...
      save () {
        if (this.editedIndex > -1) {
         ...
         this.loading=true
          axios.put(`/auth/roles/${this.editedItem.id}`, { name : this.editedItem.name })
         .then(response=>{
           this.text = "Record Updated Successfully";
           this.snackbar = true;
          Object.assign(this.roles.data[index], response.data.role, this.loading=false)
         ...
         .catch(error =>{
           console.log(error)
            ...
            this.snackbar = true;
              ...
              this.loading=true
              axios.post('/auth/roles', { name : this.editedItem.name })
            .then(response =>{
