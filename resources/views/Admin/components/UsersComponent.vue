<template>
    ...
    >
                                    ...
                                    <v-row v-if="editedIndex == -1">
                                        ...
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field
                                                ...
                                                value="C_Password"
                                                v-model="
                                                    editedItem.confirmationpassword
                                                "
                                            >
                                            </v-text-field>
                                        </v-col>
        ...
        <v-snackbar v-model="snackbar" color="pink" :timeout="0">
            <div>{{ text }}</div>
...
export default {
    ...
    computed: {
        ...
        passwordMatch() {
            if (
                this.editedItem.password != this.editedItem.confirmationpassword
            ) {
    ...
    methods: {
        ...
        updateRole(item) {
            const index = this.users.data.indexOf(item);
            axios
                .post("auth/user/role", { role: item.role, user: item.id })
                .then(response => {
                    this.text =
        ...
        deleteAll() {
            ...
            if (decided) {
                this.loading = true;
                axios
                    .post("auth/users/delete", { users: this.selected })
                    .then(response => {
        ...
        paginate(event) {
            ...
            const orderBy =
                this.options.sortDesc.length > 0 && this.options.sortDesc[0]
                    ...
                    : "desc";
            axios
                .get(`/auth/users?page=${event.page}`, {
                    params: {
        ...
        save() {
            if (this.editedIndex > -1) {
                ...
                this.loading = true;
                axios
                    .post("/auth/users", this.editedItem)
                    .then(response => {
                        this.loading = false;
